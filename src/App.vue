...
function corrigirPreco(id) {
  posicaoProduto.value = produtos.value.findIndex(p => p.id === id);
...
<template>
  <div class="container">
    <ul>
      <ProdutoChild v-for="produto in produtos" :key="produto.id" :id="produto.id" :nome="produto.nome" :preco="produto.preco" 
